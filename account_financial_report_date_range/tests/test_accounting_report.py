...
class TestAccountingReport(TransactionCase):
    def setUp(self):
        ...
        p_type = self.env["date.range.type"].create(
            {"name": "Fiscal Period", "allow_overlap": False}
        )
    ...
    def test_accounting_report(self):
        bs = self.env.ref("account.account_financial_report_balancesheet0")
        wiz = self.env["accounting.report"].create({"account_report_id": bs.id})
